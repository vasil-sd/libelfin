...
private:
  struct ml_wrapped
  {
    ml_wrapped(Ptr x, size_t t) : tag(t), p(std::move(x)) {}
    size_t tag;
    Ptr p;
  };
...
public:
  ...
  static char const* name() { return ml_name<T>(); }
  ...
  static value alloc(Ptr p, size_t tag = 0) // copy is ok
  {
    ...
    ml_wrapped* ml = new ml_wrapped(std::move(p),tag); //(ml_wrapped*)caml_stat_alloc(sizeof(ml_wrapped));
    Wrapped_val(v) = ml;
...
template<class T, class Ptr>
size_t wrapped<T, Ptr>::count_ = 0;

template<class T>
struct wrapped_ptr : public wrapped<T, std::unique_ptr<T> >
{
  typedef wrapped<T, std::unique_ptr<T> > base;
  ...
  {
    return base::alloc(std::unique_ptr<T>(p), tag);
  }
}; // wrapped_ptr
